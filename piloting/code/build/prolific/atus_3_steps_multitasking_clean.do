...
***** Load and prepare
insheet using "/Users/leolab/Downloads/atus-3steps-time-improved_January 5, 2024_07.39.csv", names clear
count
...
br responseid product live_without hours if missing(live_without) & !missing(hours) & product < 996 

preserve
	keep if missing(live_without) & !missing(hours) & product < 90 //
	tab product // only 'other' categories are not asked in live without and asked in time allocation
restore

drop if missing(live_without) & !missing(hours) & product < 90 
...
drop if product == 4 //REMOVE FOR NOW
tab product
tab responseid

* Data structure = responseid x product ( x 89)
